...
- [ ] Уменьшать количество TypeScript ошибок до green `typecheck/build` (батчами, в процессе).
  - [x] Батч 1: cleanup неиспользуемых импортов/переменных в `src/web/*`.
  - [ ] Батч 2: исправление типовых контрактов в `src/tools/*`.
  - [ ] Батч 3: исправление строгих типов в `src/agent/*` и `src/runtime/*`.
